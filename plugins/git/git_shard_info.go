...
var regMapperNewReadShard = gio.RegisterMapper(newReadShard)

func init() {
	gob.Register(shardInfo{})
	gob.Register(newShardInfo{})
}
...
func (s *newShardInfo) decode(b []byte) error {
	dec := gob.NewDecoder(bytes.NewReader(b))
	return dec.Decode(s)
}

func (s *newShardInfo) encode() ([]byte, error) {
	var b bytes.Buffer
	enc := gob.NewEncoder(&b)
	if err := enc.Encode(s); err != nil {
		return nil, errors.Wrap(err, "could not encode shard info")
...
func (s *newShardInfo) ReadSplit() error {
	log.Printf("reading %s from repo: %s", s.DataType, s.RepoPath)
